...
public class TLSConnector {
    ...
    public static final String NAME_FORMAT = "CN=P%d, OU=IFX, O=IAIK, C=AT";
    public static final Duration DEFAULT_CONNECTION_TIMEOUT = Duration.ofMinutes(1);
    private static final int PARTY_ID_BYTES = 1;
    private static final Logger logger = LoggerFactory.getLogger(TLSConnector.class);
    ...
    TLSConnector(NetworkConfiguration conf, Duration timeout, SocketFactory socketFactory,
    ...
    private Map<Integer, Socket> connectServer(final NetworkConfiguration conf) throws IOException {
        ...
        if (conf.getMyId() > 1) {
            try (SSLServerSocket server = (SSLServerSocket) serverFactory.createServerSocket(conf.getMe().getPort())) {
