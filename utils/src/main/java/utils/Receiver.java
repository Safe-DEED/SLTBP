...
class Receiver {
    ...
    Receiver(Socket sock) {
        ...
        try {
            ...
            throw new RuntimeException("Unable to get inputstream from socket.");
        }
    ...
    byte[] pollMessage(Duration timeout) {
        byte[] received;
        try{
            received = queue.poll(timeout.toMillis(), TimeUnit.MILLISECONDS);
        } catch (InterruptedException e){
            throw new RuntimeException("Receive interrupted");
        }
        return received;
    }
